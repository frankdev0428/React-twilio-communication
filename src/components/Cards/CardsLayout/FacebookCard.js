...
const MessageItemCard = ({ className, children, type, facebook, ...other }) => {
  const { body, createdDate } = facebook;
  ...
  return (
    <div className={cx(styles.messageItemWrapper)}>
      <Typography className={cx(styles.messageTimestamp, addClass(type), visible ? styles.timeShow : "")}>
        <MDAvatar src={MessengerIcon} alt="Messenger" sx={{width: '24px', height: '24px', mr: 1}} />
        {`${
          type === "received" ?
            (customer ? customer.firstName : "Customer") :
            "I"
        } sent a Messenger chat - ${moment(createdDate).fromNow()}`}
      </Typography>
      <div
